...
void TCPReceiver::segment_received_impl(const TCPHeader &header, string data) {
    ...
    if(!_syn_recved && header.syn){ //如果没有收到过syn报文且当前报文含syn
        ...
        _reassembler.push_substring(move(data), 0, eof); //第一个包，直接插入0的位置即可
    ...
    _reassembler.push_substring(move(data), stream_index, eof); //只有fin报文能够设置eof位
}

void TCPReceiver::segment_received(const TCPSegment &seg) {
    segment_received_impl(seg.header(), seg.payload().copy());
}

void TCPReceiver::segment_received(TCPSegment &&seg) {
    segment_received_impl(seg.header(), seg.payload().copy());
}
